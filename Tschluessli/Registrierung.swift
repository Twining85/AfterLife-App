...
struct Registrierung: View {
  ...
  private let registrierungHintergrund = Color.appCard
  private let registrierungAkzent = Color.appAccent
  private let registrierungKarte = Color.appRaisedCard
  private let registrierungTextPrimaer = Color.appPrimaryText
  ...
  @AppStorage("direktNachRegistrierungEingeloggt") private var direktNachRegistrierungEingeloggt =
    false
  ...
  @AppStorage("wiederherstellungNeuesGeraetLaeuft") private var wiederherstellungNeuesGeraetLaeuft =
    false
    ...
    VStack(spacing: 0) {
      TabView(selection: $onboardingSeite) {
        ...
        onboardingSeiteView(
          ...
          text:
            "Es geht um mehr als Geld. Wenn etwas passiert, müssen deine Angehörigen nicht suchen oder rätseln was zu tun ist.\nSie finden genau die Informationen, die du für sie hinterlassen möchtest."
        )
        .tag(1)
      ...
      VStack(spacing: 18) {
        HStack(spacing: 9) {
          ForEach(0..<3, id: \.self) { index in
            Circle()
              .fill(
                index == onboardingSeite ? registrierungAkzent : registrierungAkzent.opacity(0.2)
              )
    ...
    GeometryReader { proxy in
      ZStack(alignment: .bottom) {
        Image(bild)
          .resizable()
          .scaledToFill()
          .frame(width: proxy.size.width, height: hoehe)
          .clipped()
        ...
        VStack(spacing: 10) {
          Text(titel)
            .font(titelgroesse >= 30 ? .title.bold() : .title2.bold())
            .fontDesign(.rounded)
    ...
    VStack(alignment: .leading, spacing: 14) {
      ...
      ForEach(VorsorgeEmpfaenger.allCases) { empfaenger in
        Button {
          ...
          HStack(spacing: 12) {
            Image(
              systemName: ausgewaehlterEmpfaenger == empfaenger
  ...
  private var zugangKarte: some View {
    registrierungKarteView(titel: "Zugang erstellen", systemImage: "lock.shield.fill") {
      VStack(spacing: 14) {
        ...
        Text(
          "Dein Passwort wird nicht in der App gespeichert. Der Server speichert nur einen nicht lesbaren Hash."
        )
        .font(.caption2)
  ...
  private var hinweisKarte: some View {
    registrierungKarteView(titel: "Wichtiger Hinweis", systemImage: "info.circle.fill") {
      VStack(alignment: .leading, spacing: 13) {
        Text(
          "Tschlüssli dient ausschliesslich der Organisation persönlicher Informationen und ersetzt keine Rechts-, Steuer-, Finanz- oder Vorsorgeberatung sowie keine rechtsgültigen Dokumente wie Testamente, Erbverträge, Vorsorgeaufträge oder Patientenverfügungen. Die Nutzung erfolgt in eigener Verantwortung."
        )
        .font(.caption)
        .foregroundStyle(.secondary)
        .fixedSize(horizontal: false, vertical: true)
  ...
  private func registrierungKarteView<Content: View>(
    titel: String, systemImage: String, @ViewBuilder content: () -> Content
  ) -> some View {
  ...
  private var registrierungErlaubt: Bool {
    akzeptiertDisclaimer && akzeptiertNutzungsbedingungen && registrierungsEmailIstFormalGueltig
      && bereinigtesPasswort.count >= 12 && passwoerterStimmenUeberein
  }
  ...
  private var passwoerterStimmenUeberein: Bool {
    !bereinigtePasswortWiederholung.isEmpty && bereinigtesPasswort == bereinigtePasswortWiederholung
  }
    ...
    Task {
      do {
          ...
          meldung.localizedCaseInsensitiveContains("Konto besteht bereits")
        {
          ...
          fehlermeldung =
            "Die Registrierung konnte nicht abgeschlossen werden: \(error.localizedDescription)"
        }
      }
    }
  }
  ...
  private func bestehendesKontoNachRecoveryAbschliessen(_ sitzung: CloudKontoSitzung, email: String)
  {
    ...
    do {
      ...
      fehlermeldung =
        "Die lokalen Kontodaten konnten nicht gespeichert werden: \(error.localizedDescription)"
    }
  }

  private func speichereRegistrierungsdaten(
  ...
  private func erstelleDossierFallsNoetig(fuer profil: ProfilModell, email: String) {
    if let vorhandeneDossierID = profil.dossierID {
      ...
      let existiertLokal =
        (try? modelContext.fetch(FetchDescriptor<DossierModell>()))?.contains {
